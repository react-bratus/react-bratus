...
class ASTParser {
    constructor(options) {
        ...
        console.log(`[ASTParser] Parser initialized with path: ${this.options.rootFolderPath}`);
    }
    parse() {
        return new Promise((resolve) => {
            console.log(`[ASTParser] Parsing project`);
            this.getFilesAndDirectories().then(async (files) => {
                ASTParser.logEntry(`[ASTParser] Traversing files: [${files.join(',\n')}]`);
    ...
    writeDataToFile(graphData) {
        ...
        console.log(`[ASTParser] Writing data to ${this.options.pathToSaveDir}/data.json`);
    }
                ...
                (0, traverse_1.default)(ast, {
                    ...
                    Identifier({ node }) {
                        ...
                        if (isConditional && conditionIdentifier == '') {
                            ...
                            ASTParser.logEntry(`[Parser] Found first condition identifier: ${node.name}`);
                        }
                    },
                    IfStatement() {
                    ...
                    LogicalExpression() {
                        ASTParser.logEntry(`[Info] Conditional rendering by '&&' found for this componenet.`);
                        isConditional = true;
                        conditionKind = '[&&]';
                    },
                    ...
                    exit({ node }) {
                        ...
                        if (node.type == 'LogicalExpression' ||
                            ...
                            ASTParser.logEntry(`[Info] Resetting the conditional parser state (3 types)`);
                            isConditional = false;
                            conditionIdentifier = '';
                        }
